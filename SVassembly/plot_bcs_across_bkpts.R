...
Rplot_bcs <- function(bkpt_name){
  ...
  label_calc_low <-min(df_1$window_start)
  label_low<-(round(label_calc_low/100000)*100000)/1000000

  label_calc_hi <-max(df_1$window_end)
  label_hi<-(round(label_calc_hi/100000)*100000)/1000000

  #label_high<-label_low + 400000
  #label_high <-max(m1$window_end)
  break_calc<-(label_hi-label_low)/3
  ...
  p1 <- ggplot(m1,aes(x=(window_start/1000000),y=value, colour=haps)) +
    ...
    scale_x_continuous(limits = c(min(df_1$window_start)/1000000, max(df_1$window_end)/1000000), breaks=seq(label_low,label_hi,break_calc)) +
  ...
  label_calc_low2 <-min(df_2$window_start)
  label_low2<-(round(label_calc_low2/100000)*100000)/1000000
  label_calc_hi2 <-max(df_2$window_end)
  label_hi2<-(round(label_calc_hi2/100000)*100000)/1000000
  break_calc2<-(label_hi2-label_low2)/3
  ...
  p2 <- ggplot(m2,aes(x=(window_start/1000000),y=value, colour=haps)) +
    ...
    scale_x_continuous(limits = c(min(df_2$window_start)/1000000, max(df_2$window_end)/1000000), breaks=seq(label_low2,label_hi2,break_calc2)) +
    scale_y_continuous(breaks=seq(0,label_y,20))

  #label = textGrob("chr3 (Mb)",hjust=0.5, gp = gpar(fontsize = 40))
  ...
  full_plot<-grid.arrange(mylegend, arrangeGrob(p1 + theme(legend.position="none"),p2,ncol=2,nrow=1,widths=c(60, 52), heights=c(1)), nrow=2, heights=c(1,10))
